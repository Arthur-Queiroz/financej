...
<template>
  ...
  >
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      ...
      <div
        class="hidden md:flex items-center gap-1.5 text-sm"
        style="color: var(--ink-3);"
      >
        <UIcon
    ...
    >
      ...
      <div
        v-else
        class="grid md:grid-cols-[1.4fr_1fr] gap-7 items-center"
      >
        <div>
          ...
          <div class="flex gap-5 mt-5 text-sm flex-wrap">
            <div>
              <div style="color: var(--ink-3); margin-bottom: 4px;">
                {{ t('dashboard.income') }}
              </div>
              <div class="mono font-medium text-base">
                {{ fmtBRL(summary?.totalIncome ?? 0) }}
              </div>
            </div>
            <div
              class="hidden md:block w-px"
            ...
            <div>
              ...
              <div
                class="mono font-medium text-base"
            ...
            <div class="hidden md:block">
              <div style="color: var(--ink-3); margin-bottom: 4px;">
                {{ expenses?.length ?? 0 }} {{ t('dashboard.entries') }}
              </div>
    ...
    <section class="grid md:grid-cols-[1.1fr_1fr] gap-5">
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            ...
            <div
              ...
              style="color: var(--ink-3);"
            >
              {{ categoryItems.length }} {{ t('dashboard.active_categories') }}
            </div>
          </div>
        </div>
        <div
        ...
        <div
          ...
          class="grid md:grid-cols-[160px_1fr] gap-6 items-center"
        >
          <div
      ...
      >
        <div class="flex items-center justify-between mb-2">
          ...
          <button
            ...
            @click="showAllExpensesModal = true"
          >
            {{ t('dashboard.see_all') }} <UIcon
        ...
        <template v-else>
          <template
            v-for="(exp, i) in (expenses ?? []).slice(0, 7)"
            :key="exp.id"
          >
            <ExpenseRow :exp="exp" />
            <div
              v-if="i < Math.min((expenses?.length ?? 0), 7) - 1"
              class="fm-divider"
              style="opacity: 0.6;"
            />
          </template>
    ...
    <button
      class="fm-btn fm-btn--ghost w-full md:hidden"
      @click="handleExportDashboard"
    >
      <UIcon
        name="lucide:download"
