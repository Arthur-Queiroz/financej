...
<template>
  <div class="p-4 md:p-7 pb-24 md:pb-7 flex flex-col gap-5 md:gap-6" style="color: var(--ink);">
    ...
    <!-- Summary strip -->
    <section class="fm-card grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6" style="padding: 18px;">
      <div>
        <div class="fm-label mb-1.5">{{ t('income.total_monthly') }}</div>
        <div class="mono" style="font-size: 28px; font-weight: 500; letter-spacing: -0.025em; color: var(--positive);">
          {{ fmtBRL(totalMonthly) }}
        </div>
      </div>
      <div class="md:border-l md:border-[var(--border)] md:pl-6 pt-5 md:pt-0 border-t md:border-t-0 border-[var(--border)]">
        ...
        <div style="font-size: 28px; font-weight: 500; letter-spacing: -0.025em;">{{ recurringIncomes.length }}</div>
      </div>
      <div class="md:border-l md:border-[var(--border)] md:pl-6 pt-5 md:pt-0 border-t md:border-t-0 border-[var(--border)]">
        <div class="fm-label mb-1.5">{{ t('income.one_time_count') }}</div>
        <div style="font-size: 28px; font-weight: 500; letter-spacing: -0.025em;">{{ oneTimeIncomes.length }}</div>
      </div>
    </section>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
        ...
        <div class="md:col-span-2">
          ...
          <select v-model="recurrence" class="fm-input">
            ...
            <option value="ONE_TIME">{{ t('income.one_time') }}</option>
          </select>
        </div>
        <div class="md:col-span-2">
          <label class="fm-label mb-2">{{ t('income.label') }}</label>
          <input v-model="label" class="fm-input" :placeholder="labelPlaceholder" />
        </div>
        <div>
    ...
    <section class="flex flex-col gap-3">
      <div v-if="recurringIncomes.length > 0" class="fm-label" style="padding-left: 2px;">{{ t('income.recurring_section') }}</div>
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap items-center gap-3 text-xs" style="color: var(--ink-3);">
              ...
              <span class="flex items-center gap-1"><UIcon name="lucide:calendar" class="w-3 h-3" /> {{ t('income.since') }} {{ new Date(inc.effectiveFrom).toLocaleDateString(currentLocale?.value?.code || 'pt-BR', { month: 'short', year: 'numeric' }) }}</span>
    ...
    <!-- One-time incomes -->
    <section v-if="oneTimeIncomes.length > 0" class="flex flex-col gap-3">
      <div class="fm-label" style="padding-left: 2px;">{{ t('income.one_time_section') }}</div>
      ...
      >
        <div class="flex items-start gap-3">
          ...
          >
            <UIcon name="lucide:circle-dollar-sign" class="w-5 h-5" />
          </div>
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-2 mb-2">
              <div style="font-size: 15px; font-weight: 600;">{{ inc.label }}</div>
            ...
            <div class="flex flex-wrap items-center gap-3 text-xs" style="color: var(--ink-3);">
              <span class="flex items-center gap-1"><UIcon name="lucide:calendar" class="w-3 h-3" /> {{ t('income.received_on') }} {{ formatDate(inc.effectiveFrom) }}</span>
              <span
              ...
              >
                <UIcon name="lucide:zap" class="w-3 h-3" /> {{ t('income.one_time_badge') }}
              </span>
            </div>
          </div>
          <button class="fm-btn fm-btn--icon fm-btn--subtle flex-shrink-0" style="width: 36px; height: 36px;" @click="del(inc.id)">
