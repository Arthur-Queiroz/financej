...
watch(() => user.value?.fullName, (v) => { if (v) displayName.value = v })
...
<template>
  <div class="p-4 md:p-7 pb-24 md:pb-7 flex flex-col gap-4 md:gap-6" style="color: var(--ink);">

    <div class="md:grid" style="grid-template-columns: 220px 1fr; gap: 28px;">
      <!-- Sub-nav (desktop) -->
      <div class="hidden md:block">
        <div class="text-sm mb-1" style="color: var(--ink-3);">{{ t('settings.title') }}</div>
        <h1 style="font-size: 24px; font-weight: 600; letter-spacing: -0.025em; margin: 0 0 18px;">{{ t('settings.account') }}</h1>
      ...
      <div class="flex flex-col gap-5">
        ...
        <!-- Mobile header -->
        <div class="md:hidden">
          <h1 style="font-size: 24px; font-weight: 600; letter-spacing: -0.025em; margin: 0 0 12px;">{{ t('settings.title') }}</h1>
          <select
            v-model="activeSection"
            class="fm-input w-full"
            style="font-size: 14px;"
          >
            <option v-for="item in subNav" :key="item" :value="item">{{ item }}</option>
          </select>
        </div>
        ...
        <section class="fm-card p-4 md:p-6">
          <div class="text-sm md:text-base font-semibold mb-1">{{ t('settings.profile') }}</div>
          <div class="text-xs mb-4" style="color: var(--ink-3);">{{ t('settings.profile_subtitle') }}</div>
          ...
          <div class="flex flex-col md:flex-row gap-4 items-start">
            ...
            <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input v-model="displayName" class="fm-input" />
              </div>
              <div>
          ...
          <!-- Save button -->
          <div class="flex justify-end mt-4 pt-4" style="border-top: 1px solid var(--border);">
            <button
              class="fm-btn fm-btn--primary w-full md:w-auto"
              :disabled="savingProfile"
              @click="saveProfile"
            >
              <UIcon v-if="savingProfile" name="lucide:loader-2" class="w-4 h-4 animate-spin" />
              <template v-else>
                <UIcon name="lucide:check" class="w-4 h-4" /> {{ t('settings.save_changes') }}
              </template>
            </button>
          </div>
        </section>
        ...
        <!-- Appearance -->
        <section class="fm-card p-4 md:p-6">
          <div class="text-sm md:text-base font-semibold mb-1">{{ t('settings.appearance') }}</div>
          <div class="text-xs mb-4" style="color: var(--ink-3);">{{ t('settings.appearance_subtitle') }}</div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <!-- Theme -->
            <div>
              <div class="fm-label mb-2.5">{{ t('settings.theme') }}</div>
              <div class="grid grid-cols-3 gap-2">
                ...
                >
                  ...
                  <span class="text-xs font-medium flex items-center gap-1">
                    <UIcon :name="mode === 'light' ? 'lucide:sun' : mode === 'dark' ? 'lucide:moon' : 'lucide:monitor'" class="w-3 h-3" />
            ...
            <!-- Accent -->
            <div>
              <div class="fm-label mb-2.5">{{ t('settings.accent') }}</div>
        ...
        <!-- Locale -->
        <section class="fm-card p-4 md:p-6">
          <div class="text-sm md:text-base font-semibold mb-1">{{ t('settings.locale') }}</div>
          <div class="text-xs mb-4" style="color: var(--ink-3);">{{ t('settings.locale_subtitle') }}</div>
        ...
        <!-- Privacy -->
        <section v-if="activeSection === t('settings.privacy')" class="fm-card p-4 md:p-6">
          <div class="text-sm md:text-base font-semibold mb-1">{{ t('settings.privacy') }}</div>
          <div class="text-xs mb-4" style="color: var(--ink-3);">{{ t('settings.privacy_subtitle') }}</div>
          <div class="text-center py-8 md:py-12" style="color: var(--ink-3);">
            <UIcon name="lucide:lock" class="w-10 h-10 md:w-12 md:h-12 mx-auto mb-3 opacity-30" />
            <p class="text-sm">{{ t('settings.coming_soon') }}</p>
          </div>
        </section>

        <!-- Notifications -->
        <section v-if="activeSection === t('settings.notifications')" class="fm-card p-4 md:p-6">
          <div class="text-sm md:text-base font-semibold mb-1">{{ t('settings.notifications') }}</div>
          <div class="text-xs mb-4" style="color: var(--ink-3);">{{ t('settings.notifications_subtitle') }}</div>
          <div class="text-center py-8 md:py-12" style="color: var(--ink-3);">
            <UIcon name="lucide:bell" class="w-10 h-10 md:w-12 md:h-12 mx-auto mb-3 opacity-30" />
            <p class="text-sm">{{ t('settings.coming_soon') }}</p>
          </div>
        </section>
        ...
        <div class="flex justify-end">
          <SignOutButton>
            <button class="fm-btn fm-btn--danger fm-btn--sm">
              <UIcon name="lucide:log-out" class="w-3.5 h-3.5" /> {{ t('settings.sign_out') }}
            </button>
          </SignOutButton>
        </div>
      </div>
