...
<template>
  <div
    id="savings-content"
    class="flex flex-col gap-6 p-7 pb-24 md:pb-7"
  >
    <!-- Header -->
    <header class="flex items-start justify-between">
      <div>
        ...
        <p
          ...
          style="color: var(--ink-3); max-width: 500px;"
        >
          {{ t('savings.subtitle') }}
        </p>
      </div>
    ...
    <!-- Summary strip -->
    <section
      class="fm-card fm-card--glow"
      style="padding: 24px; display: grid; grid-template-columns: 1fr; gap: 20px;"
    ...
    >
      ...
      <div>
        ...
        <div
          class="mono"
          style="font-size: 32px; font-weight: 500; letter-spacing: -0.025em; color: var(--accent);"
        >
          {{ fmtMoney(totalSaved) }}
        </div>
      </div>
      <!-- Monthly reserved -->
      <div
        style="border-left: 1px solid var(--border); padding-left: 24px;"
        class="hidden md:block"
      >
        <div class="fm-label mb-1.5">
          {{ t('savings.monthly_reserved') }}
        </div>
        <div
          class="mono"
          style="font-size: 24px; font-weight: 500;"
        >
          {{ fmtMoney(monthlyReserved) }}
        </div>
      </div>
      <!-- Active vaults -->
      <div
    ...
    >
      ...
      <p
        ...
        style="color: var(--ink-3); max-width: 280px;"
      >
        {{ t('savings.no_vaults') }}
      </p>
    </div>
    ...
    >
      ...
      >
        ...
        <div class="flex items-baseline gap-2">
          <span
            class="mono"
            style="font-size: 28px; font-weight: 500; letter-spacing: -0.02em;"
          >
            {{ fmtMoneyFull(vault.current) }}
          </span>
          <span
            v-if="vault.target"
            style="font-size: 13px; color: var(--ink-3);"
          >
        ...
        <div v-if="vaultPct(vault) !== null">
          ...
          >
            ...
            <span
              v-if="monthsToGoal(vault) === 0"
              style="color: var(--accent);"
            >{{ t('savings.goal_reached') }}</span>
        ...
        <div
          ...
          style="color: var(--ink-3); padding: 4px 0;"
        >
          <UIcon
            name="lucide:flag"
            class="w-3 h-3"
        ...
        >
          <button
            class="fm-btn fm-btn--subtle fm-btn--sm"
            style="color: var(--ink-3);"
            @click="openWithdraw(vault)"
          >
            <UIcon
