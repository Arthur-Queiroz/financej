    ...
    <template #content>
      <div style="background: var(--surface); border-radius: 24px; border: 1px solid var(--border); overflow: hidden;">
        ...
        <div class="flex justify-between items-start" style="padding: 24px 24px 0;">
          <div>
            <div class="fm-label mb-1.5">{{ t('savings.deposit_subtitle') }} {{ vault?.name }}</div>
            <h2 style="font-size: 20px; font-weight: 600; letter-spacing: -0.02em; color: var(--ink); margin: 0;">{{ t('savings.deposit_title') }}</h2>
          </div>
        ...
        <div v-if="vault" class="flex items-center gap-2.5" style="padding: 14px 24px;">
          ...
          <div>
            ...
            <div v-if="vault.target" style="font-size: 12px; color: var(--ink-3);">
        ...
        <div style="padding: 0 24px 8px; display: flex; flex-direction: column; gap: 16px;">
          ...
          <div>
            ...
            <input v-model="note" class="fm-input" :placeholder="t('savings.deposit_note_placeholder')" />
          </div>
        </div>

        <!-- Footer -->
        <div class="flex justify-end gap-2.5" style="padding: 20px 24px; border-top: 1px solid var(--border); margin-top: 8px;">
          <button class="fm-btn fm-btn--ghost" @click="open = false">{{ t('savings.deposit_cancel') }}</button>
          <button class="fm-btn fm-btn--primary" :disabled="loading || !amountStr" @click="submit">
            <UIcon v-if="loading" name="lucide:loader-2" class="w-4 h-4 animate-spin" />
            <template v-else>
              {{ t('savings.deposit_save') }}
              <UIcon name="lucide:piggy-bank" class="w-4 h-4" />
            </template>
          </button>
        </div>
      </div>
