  ...
  >
    <template #content>
      <div style="display: flex; flex-direction: column; height: 100%;">
        ...
        <!-- Content -->
        <div class="modal-scroll-content" style="padding: 24px; overflow-y: auto; flex: 1; min-height: 0;">
          <div v-if="loading" class="flex justify-center py-12">
            <UIcon name="lucide:loader-2" class="w-6 h-6 animate-spin" style="color: var(--ink-3);" />
          </div>

          <div v-else-if="!expenses || expenses.length === 0" class="text-center py-12">
            <UIcon name="lucide:inbox" class="w-12 h-12 mx-auto mb-3" style="color: var(--ink-mute);" />
          ...
          <div v-else>
            <template v-for="(exp, i) in expenses" :key="exp.id">
              <ExpenseRow :exp="exp" :show-date="true" />
              <div v-if="i < expenses.length - 1" class="fm-divider" style="opacity: 0.4;" />
            </template>
          </div>
        </div>
      </div>
